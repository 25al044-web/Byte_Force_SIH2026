...
offline, screening continues and the local audit row is preserved locally rather
than claiming an on-chain transaction.

The Trusted Identity Registry stores baseline demonstration identity records.
Registry writes produce a canonical SHA-256 metadata hash, which is stored locally
and anchored in AuditTrail when the chain is available. Verify screening records with `POST /api/audit/{id}/verify`.
